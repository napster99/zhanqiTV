  ...
  var ui = {
    ...
    renderDefaultData : function(size) {
      ...
      for(var i=0; i<size; i++) {
        htmlStr += '<li tapmode="active" onclick="enterRooms(this);">'
                ...
                + '      <p class="anchor"><i class="icon-m icon-boy"></i><span class="js-nickname">加载中...</span>'
                + '      </p>'
                + '    </div>'
                + '</li>'
      }
    ...
    loadData : function(first, noPlacehoder) {
      ...
      , 'page': self.pageNow
        ...
        if(noPlacehoder) {
          self.renderNoPlacehoderData(data['rooms']);
        }else{
          self.renderRealData(data['rooms'], defaultCon, first);
        }
    ...
    renderRealData : function(data, con, first) {
      ...
      con.each(function(idx){
        ...
        if(idx < i){
          var id = data[idx]['id'];
          var bpic = data[idx]['bpic'] + '?v=' + (+new Date());
          ...
          // if(data[idx]['gameId'] == 45 && api.systemType === 'ios') {   //游戏放映室  为了审核而过滤
          //   $self.remove();  
          // }
      ...
      var imgLen = con.find('img.game-pic').length >= 8 ? 8 : con.find('img.game-pic').length;
      var loadCount = 0;

      for(var i=0; i<imgLen; i++) {
        con.find('img.game-pic').eq(i)[0].onload = function() {
          loadCount++;
          if(loadCount >= imgLen && first) {
    ...
    renderNoPlacehoderData : function(data) {
      var lis = ui.$liveList.find('li'), len = data.length;
      if(lis.length >= len) {
